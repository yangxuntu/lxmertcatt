...
valid_batch_size = 1024 if args.multiGPU else 512
...
class InputFeatures(object):
    ...
    def __init__(self,
                 ...
                 visual_feats, visual_masks, obj_labels,
                 is_matched, ans, random_index):
        ...
        self.visual_masks = visual_masks
...
def random_word(tokens, tokenizer, random_index):
    """
    Masking some random tokens for Language Model task with probabilities as in the original BERT paper.
    :param tokens: list of str, tokenized sentence.
    :param tokenizer: Tokenizer, object used for tokenization (we need it's vocab here)
    :param random_index: if 0, mask sentence, if 1, do not mask
    ...
    for i, token in enumerate(tokens):
        ...
        if prob < ratio and random_index == 0:
            prob /= ratio
...
def random_feat(feats, random_index):
    """
    :param random_index: if 1, mask image, if 0, do not mask
    """
    ...
    for i in range(len(feats)):
        prob = random.random()
        # mask token with probability
        if prob < args.obj_mask_rate and random_index == 1:
...
def convert_example_to_features(example: InputExample, max_seq_length, tokenizer)->InputFeatures:
    ...
    #if random_index = 0, mask sentence, random_index = 1, mask image
    random_index = np.random.randint(2)

    # Ge random words
    masked_tokens, masked_label = random_word(tokens, tokenizer, random_index)
    ...
    # Mask Image Features:
    masked_feat, feat_mask = random_feat(feat, random_index)
    obj_confs = feat_mask
    attr_confs = feat_mask
    visn_masks = 1-feat_mask
...
class LXMERT:
    def __init__(self, max_seq_length):
        ...
        if args.multiGPU:
            ...
            self.multiGPU = 0
            # self.model = nn.DistributedDataParallel(self.model)
    ...
    def forward(self, examples):
        ...
        visual_mask = torch.tensor([f.visual_masks for f in train_features], dtype=torch.long).cuda()
        ...
        loss, losses, ans_logit = self.model(
            ...
            feats, pos, obj_labels, matched_labels, ans, visual_mask
        )
        if self.multiGPU:
            loss = loss.mean()
    ...
    def train(self, train_tuple: DataTuple, eval_tuple: DataTuple):
        ...
        if args.start_from >0 and args.pretraining_index == 0:
            start_path = os.path.join(args.output, "Epoch%s_LXRT.pth" % format(int(args.start_from), '02'))
