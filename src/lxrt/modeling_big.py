...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, M = None):
        ...
        if self.bert_type == 'ft':
            ...
            FTattention_output = self.FTattention(hidden_states, M, attention_mask)  # Q K V
...
class LXRTEncoder(nn.Module):
    ...
    def forward(self, lang_feats, lang_attention_mask,
        ...
        if self.bert_type == 'ft':
            # nbatches = visn_feats.size(0)
            # M_visn = self.MV.unsqueeze(0).expand(nbatches, -1, -1)
            # M_lang = self.ML.unsqueeze(0).expand(nbatches, -1, -1)
            ...
            for layer_module in self.layer:
                if t == 1:
                    lang_feats, M_lang = layer_module(lang_feats, None, M_lang)
                else:
                    lang_feats, M_lang = layer_module(lang_feats, lang_attention_mask, M_lang)
                t += 1
            ...
            for layer_module in self.r_layers:
                if t == 1:
                    visn_feats, M_visn = layer_module(visn_feats, None, M_visn)
                else:
                    visn_feats, M_visn = layer_module(visn_feats, visn_attention_mask, M_visn)
                t += 1
